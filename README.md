...
- **Data** — the UI loads a bundled synthetic dataset (generated by the
  `SyntheticImporter`, `npm run export:fixture`) behind a single `loadDataset()`
  seam, so it runs with zero setup and is e2e-testable without a backend. That
  seam is where a live `GET /api/dataset` fetch drops in later.

```bash
npm run dev:frontend     # Vite dev server → http://localhost:5173
npm run export:fixture   # regenerate the bundled dataset from the importer
